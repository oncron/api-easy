...
exports.describe = function (text) {
  return {
    ...
    // 4. Assert a XML result: `suite.expect('xmlComparisonFile', true)`
    //
    expect: function (/* [text, code, result, assert, xml] */) {
      var args = Array.prototype.slice.call(arguments),
          text, code, result, test, xml, context;
      ...
      args.forEach(function (arg) {
        switch (typeof(arg)) {
          ...
          case 'boolean': xml = arg; break;
        }
      });
      
      context = this._currentTest(this.current);
     
      // When looking for an xml result, the a filename with the xml results
      // must be provided
      if (xml) {
        if (!text) {
          throw new Error('Expected result file is required when specifying XML comparison.');
        }
        if (test) {
          throw new Error('Cannot specifiy a custom test asertion function when specifying XML comparison.');
        }
      } else {
      ...
      if (xml && text) {
        context['should respond with XML results in file ' + text] = function (err, res, body) {
          assert.isNull(err);
          var fileResult = fs.readFileSync(text, 'utf8');
          assert.deepEqual(body, fileResult);
        };
      }
    ...
    _request: function (/* method [uri, data, params] */) {
      var self    = this,
          ...
          // Also allow data to be a string that will go into the body of the
          // request
          if (!data) data    = typeof args[0] === 'string' && args.shift();
      ...
      //
      // If the user has provided data, 
      //  it may be JSON, in which case set it to the `body` property of the options. 
      //  or it may be a stirng, in which case, add to the `body` property
      //
